...
注意 subtle 字段含义：
- true  = 伏笔模式：静默成形，玩家看不到[世界涌现]通告；模块只产出稀疏的感官现象，
         不影响人口/信仰/科技标签/倾向；像背景音里的诡异冒头，不像被宣告的新机制。
- false = 显形模式：正常通告并允许影响世界。
现实层（仪式/贸易/战争等）任何时候都该 false（人能直接读懂，没必要藏）。
超自然层（异象/灵/其他神明）若世界尚未成熟必须 true；成熟后才可以 false。"""
